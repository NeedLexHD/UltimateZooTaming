...
public class KeeperTaskLayer extends GeoRenderLayer<ZooKeeperEntity> {
    ...
    public void render(PoseStack pose, ZooKeeperEntity keeper, BakedGeoModel model,
                       ...
                       MultiBufferSource buffer, net.minecraft.client.renderer.texture.OverlayTexture overlay,
        ...
        if (task == KeeperTask.IDLE) return;      // rien a signaler
        if (keeper.isSleeping()) return;          // on ne reveille pas les dormeurs
        ...
        pose.pushPose();
        // Au-dessus de la tete, orientee vers la camera
